...
- [x] [P3] **`.prose-cv` interlignage** — actuellement 1.7, à homogénéiser à 1.6 avec le corps global (page /about uniquement) · _Origine : revue Agathe_ · `src/styles/global.css`
...
- [x] [P1] **Centraliser le contact (anti-duplication)** — `cvParser.ts:316-320` et `cvParser.ts:533-538` hardcodent email/portfolio/linkedin/location en doublon de `src/config/site.ts` · Faire de `site.ts` la source unique, le parser doit lire depuis ce config plutôt que redéclarer · _Origine : revue Astride · piège auto-tendu_
